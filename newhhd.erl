...
recovery_history_packet(Topology,LiveLatest,LiveBefore,Cur) ->
  ...
  case mc_cursor:next(Cur) of
	 ...
	 FirstHistory -> 
  		...
  		UpdatedHistoryRecord = newhhd_history_packet_recovery(LiveLatest,LiveBefore,HistoryRecord),
  		update_db(Topology,UpdatedHistoryRecord,HistoryRecord),
  		recovery_history_packet(Topology,HistoryRecord,LiveLatest,Cur)
   end.


newhhd_history_packet_recovery(LiveLatest,LiveBefore,Track) ->
   ...
   <<Previous_lock:2/binary, Prev_Batt:3/binary,Pre_Tear:1/binary,_/binary>> = LiveLastest#track.alarm_bit,
    ...
    UT2 =
          case Pre_Tear of
                <<"0">> ->
                     case TearStatus of
                           <<"1">> ->
                                check_wirecut(UpdatedTrack1, LiveLatest#track.alarm_bit, LiveBefore#track.alarm_bit);
                           _  ->
                                UpdatedTrack1
                     end;
                <<"1">> ->
	 	     case TearStatus of
                            <<"1">> ->
                                check_wirecut1(UpdatedTrack1, LiveLatest#track.alarm_bit, LiveBefore#track.alarm_bit);
                             _ ->
                               UpdatedTrack1
                     end
           end,

    UT1 = update_report_reason(UT2, ReportReason),
