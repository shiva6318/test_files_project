...
%                  ht_logger:info("LIVE DATA INSIDE ERROR=> unit: ip = '~s' id = '~w'", [Error, AssetId]),
 ...
 % do_send_response(State,TerminalId,MsgId,MsgSerialNo);
  ...
  do_send_response(State,TerminalId,MsgId,MsgSerialNo);
  <<16#03,16#11>> ->
  ht_logger:info("11GOT RESPONSE: SIZE ~w DATA: ~s", [byte_size(MsgBody),parse_hex(MsgBody)]),
  ...
  <<16#03,16#15>> ->
  ht_logger:info("15GOT RESPONSE: SIZE ~w DATA: ~s", [byte_size(MsgBody),parse_hex(MsgBody)]),
   <<ResId:1/binary,Index:1/binary,Count:8/unsigned-integer,Rest1/binary>> = MsgBody,
     Result = string:sub_string(erlang:binary_to_list(parse_hex(Index)),2,2), 
   ...
   ht_hhd_cmd_aes:cmd_check_response(State,Count,Rest1,AssetId); %%shiva code
  <<16#03,16#13>> ->
  ht_logger:info("13GOT RESPONSE: SIZE ~w DATA: ~s", [byte_size(MsgBody),parse_hex(MsgBody)]),
